...
GuardarIncidencia = async(req, res = response ) => {
   ...
   CargarIncidencias = async(req, res = response ) => { 
    try{        
        ...
        if (individual == 0){
            sql  = "SELECT Fecha, id, Motivo Titulo, Tienda, Estado, Observaciones, C.Nombre, Prioridad FROM IncidenciasManteTiendas T JOIN Clientes C ON T.Tienda = C.Numero WHERE Fecha >='" + fechaDesdeFormateada + "' AND Fecha <='" + fechaHastaFormateada + "'" + filtroEstado + filtroTiendas + filtroPrioridad + " ORDER BY Prioridad , Fecha "
        }else{            
